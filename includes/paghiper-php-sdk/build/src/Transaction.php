...
class Transaction extends \PagHiper\PagHiper
{
    ...
    {
        $cancel_transaction = $this->paghiper->request(self::CANCEL_ENDPOINT, ['transaction_id' => $transaction_id, 'status' => 'canceled'])['cancellation_request'];
    ...
    {
        $transaction_status = $this->paghiper->request(self::STATUS_ENDPOINT, ['transaction_id' => $transaction_id])['status_request'];
    ...
    public function combineBillets(array $transactions = [], string $type = NULL)
    {
        ...
        $combined_billets = $this->paghiper->request(self::MULTIPLE_ENDPOINT, ['transactions' => $transactions, 'type_bank_slip' => $type])['status_request'];
    ...
    public function process_ipn_notification(string $notification_id, string $transaction_id, string $transaction_type)
    {
        ...
        $ipn_request = $this->paghiper->request($transaction_type == 'pix' ? self::NOTIFICATION_PIX_ENDPOINT : self::NOTIFICATION_BILLET_ENDPOINT, ['notification_id' => $notification_id, 'transaction_id' => $transaction_id])['status_request'];
