    ...
    if($already_paid) {
        ...
        switch ( $return['status'] ) {
            case "pending" :
                ...
                $order->add_order_note( __( 'PagHiper: Novo '.$gateway_name.' emitido. Aguardando compensação.', 'woo-boleto-paghiper' ) );
                if ( $paghiper_log ) {
            ...
            case "canceled" :
                    ...
                    if($return['transaction_id'] !== $paghiper_data['transaction_id']) {
                        $order->add_order_note( __( 'PagHiper: Um '.$gateway_name.' emitido para este pedido foi cancelado. Como não era o boleto mais atual, o pedido permanece aguardando pagamento.', 'woo-boleto-paghiper' ) );
                    ...
                    $order->update_status( $cancelled_status, __( 'PagHiper: '.ucfirst($gateway_name).' Cancelado.', 'woo-boleto-paghiper' ) );
                    paghiper_increase_order_stock( $order, $settings );
            ...
            case "paid" :
                ...
                if(!$order->has_status( $paid_statuses )) {
                    ...
                    $order->update_status( $target_status, __( 'PagHiper: '.ucfirst($gateway_name).' pago.', 'woo-boleto-paghiper' ) );
                    ...
                    $order->add_order_note( __( 'PagHiper: '.ucfirst($gateway_name).' compensado.', 'woo-boleto-paghiper' ) );
