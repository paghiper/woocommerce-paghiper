...
class WC_PagHiper_Transaction {
	...
	public function prepare_data_for_transaction() {
		...
		if(!empty($billing_person_type)) {
			$data['payer_name'] = ($billing_person_type == 2 && !empty($this->order->billing_company)) ? $this->order->billing_company : $this->order->get_billing_first_name() . ' ' . $this->order->billing_last_name;
			...
			if(!empty($this->order->get_meta( '_billing_cnpj' )) && !empty($this->order->billing_company)) {
				...
				if(!empty($this->order->get_meta( '_billing_cnpj' )) && !empty($this->order->billing_company)) {
					...
					$data['payer_name'] = $this->order->get_billing_first_name() . ' ' . $this->order->billing_last_name;
